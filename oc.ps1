# Start OpenCode in Docker with the current directory mounted as /workspace.
# Usage:
#   oc              Start a new session
#   oc --continue   Continue the previous session for this folder
#
# Auth: On first run, OpenCode will prompt you to connect a provider.
...
$containerName = "opencode-$($folderName -replace '@', '-')"
...
if ($args -contains "--continue") {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        docker exec -it $containerName opencode --continue
    } else {
